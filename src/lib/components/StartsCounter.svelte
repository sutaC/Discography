<script lang="ts">
	...
	$: tooltip = !isLogedIn ? 'Login to star songs!' : isStared ? 'Star' : 'Unstar';
</script>

<div class="starsCounter">
	<span>{startsCount}</span>
	<IconButton on:click={handleClick} disabled={!isLogedIn} {tooltip}>
		{#if isLogedIn && isStared}
			<!-- Star - filled -->
			<svg
